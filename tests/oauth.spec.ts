...
it("re-reads the rotating token after winning the lease",async()=>{
  mocks.getSession.mockResolvedValueOnce({...session}).mockResolvedValueOnce({...session,accessToken:"concurrent-new",refreshToken:"concurrent-rotated",expiresAt:Date.now()+300000});
  expect(await accessToken(session)).toBe("concurrent-new");expect(fetch).not.toHaveBeenCalled();
});
